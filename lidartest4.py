...
class DummySerialCommunicator:
    """
    A dummy class to simulate serial communication with Arduino.
    It stores sent commands and allows checking its state.
    """
    def __init__(self, port='DUMMY_PORT', baud_rate=9600):
        self.port = port
    ...
    def connect(self):
        self.connected = True
        print(f"Dummy Arduino connected on {self.port}")
        return True

    def disconnect(self):
        self.connected = False
        print(f"Dummy Arduino disconnected from {self.port}")

    def send_command(self, command):
        if not self.connected:
            print("Dummy Arduino not connected")
            return False
        self.sent_commands.append(command)
        print(f"Dummy Arduino received: {command.strip()}")
        if command == "F\n":
            self.f_sent = True
            self.z_sent = False
        elif command == "Z\n":
...
class LidarVisualizer:
    def __init__(self, master, lidar_processor, serial_communicator):
        ...
        # Dummy Arduino Controls
        ttk.Label(self.controls_frame, text="Dummy Arduino Port:").grid(row=0, column=2, padx=5, pady=5, sticky=tk.W)
        self.arduino_port_entry = ttk.Entry(self.controls_frame, width=15)
        self.arduino_port_entry.insert(0, 'DUMMY_PORT')
        ...
        # Dummy Arduino Connection Buttons
        self.connect_arduino_button = ttk.Button(self.controls_frame, text="Connect Dummy Arduino", command=self.connect_arduino)
        self.connect_arduino_button.grid(row=1, column=2, padx=5, pady=5, sticky=tk.W+tk.E)

        self.disconnect_arduino_button = ttk.Button(self.controls_frame, text="Disconnect Dummy Arduino", command=self.disconnect_arduino, state=tk.DISABLED)
    ...
    def connect_arduino(self):
        ...
        if port:
            ...
            if self.serial_communicator.connect():
                ...
                self.update_status(f"Dummy Arduino connected on {port}")
            else:
                messagebox.showerror("Error", f"Could not connect to dummy Arduino on port {port}")
        else:
            messagebox.showerror("Error", "Please enter a dummy Arduino port.")
    ...
    def disconnect_arduino(self):
        ...
        self.update_status("Dummy Arduino disconnected")

    def on_stop_assist_changed(self):
        if self.stop_assist_var.get():
            # Check if Arduino is connected
            if self.serial_communicator.connected:
                # Check if there's a very close obstacle in front
                if self.lidar_processor.closest_distance < VERY_CLOSE_THRESHOLD:
                    # Send 'V 0\n' and 'F\n' commands
                    self.serial_communicator.send_command("V 0\n")
                    # Only send F if it hasn't been sent before
                    if not self.serial_communicator.f_sent:
                        self.serial_communicator.send_command("S 1\n")
                        self.update_command_buttons()
                        self.update_status(f"Stop assist activated: V 0 and F commands sent (very close obstacle)")
                else:
                    self.update_status(f"Stop assist activated: Very close obstacle NOT detected (distance: {self.lidar_processor.closest_distance:.2f} cm)")
            else:
                self.update_status("Stop assist activated: Arduino not connected")
        else:
    ...
    def update_display(self):
        ...
        # Simulate LIDAR data if not connected (for testing logic)
        if self.lidar_processor.lidar is None:
            simulated_scan = {}
            # Simulate some points in front
            simulated_scan[0] = 150  # Straight ahead, within threshold
            simulated_scan[10] = 180 # Slightly to the right, within threshold
            simulated_scan[-10] = 220 # Slightly to the left, outside threshold
            simulated_scan[45] = 100  # To the side, within threshold but likely out of path
            simulated_scan[-45] = 120 # To the other side

            simulated_obstacle_points = []
            min_simulated_distance = float('inf')
            min_simulated_angle = 0
            for angle, distance in simulated_scan.items():
                if -VIEW_ANGLE/2 <= angle <= VIEW_ANGLE/2:
                    if distance <= OBSTACLE_THRESHOLD:
                        lateral_position = distance * math.tan(math.radians(angle))
                        in_path = abs(lateral_position) <= (VEHICLE_WIDTH / 2)
                        simulated_obstacle_points.append((angle, distance, in_path))
                        if in_path and distance < min_simulated_distance:
                            min_simulated_distance = distance
                            min_simulated_angle = angle

            with self.lidar_processor.data_lock:
                self.lidar_processor.scan_data = simulated_scan
                self.lidar_processor.obstacle_points = simulated_obstacle_points
                if min_simulated_distance < float('inf'):
                    self.lidar_processor.closest_distance = min_simulated_distance
                    self.lidar_processor.closest_angle = min_simulated_angle
                self.lidar_processor.history.append((simulated_scan, simulated_obstacle_points, min_simulated_distance if min_simulated_distance < float('inf') else float('inf'), min_simulated_angle))
...
def main():
    try:
        # Initialize Dummy Serial Communicator
        serial_communicator = DummySerialCommunicator()
