...
class SerialCommunicator:
    def __init__(self, port=DEFAULT_ARDUINO_PORT, baud_rate=SERIAL_BAUD_RATE):
        ...
        self.f_sent = False  # Track if F has been sent
        self.z_sent = False  # Track if Z has been sent
        self.simulated_speed = 0  # Simulated vehicle speed
    ...
    def send_command(self, command):
        ...
        try:
            print(f"[SIMULATED] Sent to Arduino: {command.strip()}")
            ...
            print(f"[SIMULATED] Error sending command to Arduino: {e}")
            return False
...
class LidarVisualizer:
    ...
    def connect_arduino(self):
        ...
        if port:
            ...
            if self.serial_communicator.connect():
                ...
                self.z_command_button.config(state=tk.DISABLED)  # Disabled until F is sent
    ...
    def update_display(self):
        ...
        if (self.stop_assist_active.get() and 
            ...
            not self.stop_assist_triggered):
