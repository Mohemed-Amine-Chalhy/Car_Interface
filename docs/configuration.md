...
| `require_ack` | `CAR_INTERFACE_REQUIRE_ACK` | `true` | Must remain true for hardware; false is only for isolated simulation/protocol development |
| `ack_timeout_seconds` | `CAR_INTERFACE_ACK_TIMEOUT_SECONDS` | `0.2` | 0.05 through 10 seconds; qualification may require a stricter value |
| `heartbeat_interval_seconds` | `CAR_INTERFACE_HEARTBEAT_INTERVAL_SECONDS` | `0.1` | 0.05 through 5 seconds; must leave margin below the watchdog |
| `command_stale_seconds` | `CAR_INTERFACE_COMMAND_STALE_SECONDS` | `0.5` | Must be greater than heartbeat interval plus ACK timeout; also supplies the code-level watchdog contract |
...
sends protocol-v1 safety commands. Reducing it can make the initial handshake
race the reset; increasing it delays connection readiness. Qualify the value for
the exact board/bootloader and never use it as a substitute for safe firmware
boot behavior.

## Timing relationship

The protocol contract requires the following relationship:
...
- Do not set `require_ack = false` for a physical vehicle.
