...
# Sound Defaults
BEEP_SOUND_FILE = "beep.wav" # Name of your sound file
DEFAULT_PROXIMITY_SOUND_ENABLED = False
MAX_BEEP_DISTANCE = 150  # Start beeping when obstacle is within 150 cm
MIN_BEEP_DELAY = 0.1    # Fastest beep rate (seconds)
MAX_BEEP_DELAY = 1.0    # Slowest beep rate (seconds)
...
class LidarVisualizerFrame(ttk.Frame):
    ...
    def __init__(self, master, main_app, lidar_processor, arduino_communicator, log_callback, **kwargs):
        ...
        self._create_widgets() # This now adds the sound checkbox
        self._setup_canvas()


    def _create_widgets(self):
        # ... (Existing controls_frame for Port/Activate/Deactivate) ...
        controls_frame = ttk.LabelFrame(self, text="LIDAR Control")
        controls_frame.pack(pady=5, padx=10, fill=tk.X)
        # ... (Port Entry, Activate/Deactivate buttons) ...
        ...
        # --- Add Sound Checkbox here ---
        # Use the main_app's variable directly
        self.main_app.sound_enabled_var = tk.BooleanVar(value=self.main_app.sound_enabled_by_user)
        self.sound_checkbox = ttk.Checkbutton(
             controls_frame,
             text="Enable Proximity Sound",
             variable=self.main_app.sound_enabled_var, # Link to variable in main app
             command=self.main_app.toggle_proximity_sound # Call main app's handler
        )
        self.sound_checkbox.grid(row=1, column=2, padx=15, pady=5, sticky=tk.W)
        # Disable checkbox if sound couldn't be loaded/initialized
        if not self.main_app.beep_sound:
             self.sound_checkbox.config(state=tk.DISABLED)
             self.main_app.sound_enabled_var.set(False) # Ensure it's off if sound failed
        # --- End Sound Checkbox ---
    ...
    def update_display(self):
        ...
        # Handle Auto-Stop Assist Logic
        self.handle_auto_stop_assist(closest_distance)

        # --- Drawing on Canvas ---
        self.canvas.delete("scan", "obstacle", "closest")
        # ... (existing drawing code for scan points, obstacles, closest line) ...
        ...
        if closest_distance < float('inf'):
            ...
            if closest_distance < threshold:
                ...
                if is_braking and brake_reason == "E-Stop":
                     ...
                     text = "AUTO E-STOP ACTIVE!" if assist_enabled else "MANUAL E-STOP ACTIVE!"
                     self.canvas.itemconfig(self.warning_text, text=text, fill=RED)
                elif is_braking:
                    self.canvas.itemconfig(self.warning_text, text=f"BRAKE ACTIVE ({brake_reason})!", fill=YELLOW)
                else:
                    self.canvas.itemconfig(self.warning_text, text="E-STOP ADVISED!", fill=YELLOW)
            else:
                self.canvas.itemconfig(self.warning_text, text="")

            # --- <<< ADD PROXIMITY SOUND LOGIC HERE >>> ---
            if self.main_app.sound_enabled_by_user and self.main_app.beep_sound and pygame.mixer.get_init():
                if 0 < closest_distance <= MAX_BEEP_DISTANCE:
                    # Calculate desired delay based on distance (linear interpolation)
                    # Clamp distance to range [0, MAX_BEEP_DISTANCE] for calculation
                    clamped_dist = max(0.0, min(closest_distance, MAX_BEEP_DISTANCE))
                    # Interpolation factor (0 = closest, 1 = farthest)
                    t = clamped_dist / MAX_BEEP_DISTANCE
                    # Calculate delay: closer = MIN_DELAY, farther = MAX_DELAY
                    current_delay = MIN_BEEP_DELAY * (1 - t) + MAX_BEEP_DELAY * t

                    # Check if enough time has passed since the last beep
                    current_time = time.time()
                    if current_time - self.main_app.last_beep_time >= current_delay:
                        self.main_app.beep_sound.play()
                        self.main_app.last_beep_time = current_time
                # Optional: else: # If distance > MAX_BEEP_DISTANCE, ensure no beep is pending? (Not strictly needed)
            # --- <<< END OF PROXIMITY SOUND LOGIC >>> ---

        else:
            # No valid closest distance
            self.canvas.itemconfig(self.distance_text, text="Dist: -- cm")
            self.canvas.itemconfig(self.warning_text, text="")

        # Schedule next update
        self.update_id = self.master.after(50, self.update_display) # Reduced delay to 50ms (20 FPS) for smoother sound timing
...
class CombinedRobotApp:
    def __init__(self, root):
        ...
        self.sound_enabled_by_user = DEFAULT_PROXIMITY_SOUND_ENABLED
        self.last_beep_time = 0
        try:
            pygame.mixer.init(frequency=22050, size=-16, channels=2, buffer=512) # Initialize mixer
            self.log_message("Pygame mixer initialized.", is_info=True)
            try:
                self.beep_sound = pygame.mixer.Sound(BEEP_SOUND_FILE)
                self.log_message(f"Loaded sound file: {BEEP_SOUND_FILE}", is_success=True)
            except pygame.error as e:
                self.log_message(f"Error loading sound file '{BEEP_SOUND_FILE}': {e}. Proximity sound disabled.", is_error=True)
                self.beep_sound = None # Ensure it's None if loading failed
            except FileNotFoundError:
                self.log_message(f"Sound file '{BEEP_SOUND_FILE}' not found. Proximity sound disabled.", is_error=True)
                self.beep_sound = None # Ensure it's None if missing
        except pygame.error as e:
            self.log_message(f"Failed to initialize pygame.mixer: {e}. Sound disabled.", is_error=True)
            self.beep_sound = None # Cannot use sound
        ...
        # self.proximity_sound_enabled = self.sound_enabled_by_user and self.beep_sound is not None # Combined state - Managed by Checkbox logic now

        # --- UI Setup ---
        self._create_styles()
        self._create_main_ui() # This will now add the checkbox via LidarVisualizerFrame
      ...
      # Start polling the Xbox controller
    ...
    def toggle_proximity_sound(self):
        # This method will be called by the checkbox in LidarVisualizerFrame
        # The actual state is stored in self.sound_enabled_by_user
        # The checkbox variable itself handles the toggle
        if self.sound_enabled_var.get():
             if self.beep_sound:
                 self.log_message("Proximity sound ENABLED.", is_info=True)
                 self.sound_enabled_by_user = True
             else:
                 self.log_message("Cannot enable proximity sound: Sound file not loaded or mixer failed.", is_warning=True)
                 self.sound_enabled_var.set(False) # Force checkbox off
                 self.sound_enabled_by_user = False
        else:
             self.log_message("Proximity sound DISABLED.", is_info=True)
             self.sound_enabled_by_user = False
    ...
    def poll_controller(self):
        ...
        try:
            if self.xbox_controller.is_connected():
                ...
                if not success:
                    # Controller disconnected during update handling (already done in previous step)
                    ...
                    if event == "HAT_CHANGED":
                        ...
                        if not self.brake_active:
                         ...
                         if self.brake_active and self.brake_reason == "Controller LT":
                              ...
                              if self.arduino_communicator.send_command("Q 1"):
                                   self.update_brake_state(False, "Controller LT") # Clear specific reason
                              else:
                    ...
                    # ***** ADD THIS BLOCK *****
                         ...
                         # Check if the brake is currently active due to E-Stop
                         if self.brake_active and self.brake_reason == "E-Stop":
            ...
            # Schedule next poll even after error? Yes.
    ...
    def on_closing(self):
        ...
        if pygame.mixer.get_init():
            pygame.mixer.quit()
            self.log_message("Pygame mixer quit.")
        # --- End Add Mixer Quit ---
        pygame.quit()
        self.log_message("Pygame shut down.")
