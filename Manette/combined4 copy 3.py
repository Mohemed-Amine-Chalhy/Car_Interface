...
class XboxController:
    def __init__(self, controller_id=0, log_callback=None, arduino_communicator=None):
        ...
        self.is_connected = False  # Boolean attribute, not method
        self.controller = None
        self.controller_id = controller_id
        self.controller_name = "None"
        self.arduino_communicator = arduino_communicator  # Store referenc reference to arduino communicator
    ...
    def connect(self):
        ...
        if joystick_count <= self.controller_id:
            ...
            self.is_connected = False  # Changed from self.connected to self.is_connected
            return False
        try:
            ...
            self.is_connected = True  # Changed from self.connected to self.is_connected
            ...
            self.is_connected = False; self.controller = None; return False  # Changed from self.connected to self.is_connected
        except Exception as e:
    ...
    def disconnect(self):
        if self.controller:
            try:
                # Send stop command to Arduino when controller disconnects
                if self.arduino_communicator and hasattr(self.arduino_communicator, 'send_command'):
                    self.arduino_communicator.send_command("V 0")
                    self.log_callback("Sent stop command (V 0) to Arduino due to controller disconnect.")
    ...
    # Note: The get_hat_values method and any other methods need to be included here
    # I'm assuming your code has this method since it's referenced above
    def get_hat_values(self):
        # This is a placeholder - implement according to your existing code
        if not self.is_connected or not self.controller:
            return (0, 0)
        
        if self.USE_HAT_DPAD and self.controller.get_numhats() > self.HAT_DPAD_XBOX:
            return self.controller.get_hat(self.HAT_DPAD_XBOX)
        else:
            # Use button-based D-pad simulation
            x, y = 0, 0
            if self.dpad_button_state.get('right', False): x = 1
            if self.dpad_button_state.get('left', False): x = -1
            if self.dpad_button_state.get('up', False): y = 1
            if self.dpad_button_state.get('down', False): y = -1
            return (x, y)
    
    def set_arduino_communicator(self, arduino_communicator):
        """Set or update the Arduino communicator reference after initialization."""
        self.arduino_communicator = arduino_communicator
        self.log_callback("Arduino communicator reference updated.")
