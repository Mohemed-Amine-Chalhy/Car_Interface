...
class XboxController:
    def __init__(self, controller_id=0, log_callback=None):
        ...
        # Controller mapping (adjust if necessary for your controller model)
        self.AXIS_RIGHT_STICK_X = 0 # Usually axis 3 for horizontal on right stick
        self.AXIS_RT = 5  # Right trigger (often axis 5, range -1 to 1)
        self.AXIS_LT = 4  # Left trigger (often axis 4, range -1 to 1)
    ...
    def connect(self):
        ...
        if joystick_count <= self.controller_id:
            self.log_callback(f"Xbox controller ID {self.controller_id} not found (Count: {joystick_count}).", is_warning=True)
            self.connected = False
            return False
        ...
        try:
            ...
            self.connected = True
            self.log_callback(f"Connected to Xbox controller: {self.controller_name}")
            ...
            self.hat_values = self.controller.get_hat(self.HAT_DPAD) if self.controller.get_numhats() > self.HAT_DPAD else (0,0)
    ...
    def disconnect(self):
        ...
        self.controller = None
        self.controller_name = "None"
        self.log_callback("Xbox controller disconnected.")

    def update(self):
        """Pumps Pygame events and reads current controller state. Returns True if successful, False on error."""
        if not self.connected or not self.controller:
            return False, None # Return False if not connected

        try:
            pygame.event.pump() # Keep Pygame's event queue flowing
            ...
            self.last_hat_values = self.hat_values # Store previous before reading new
            if num_hats > self.HAT_DPAD:
                self.hat_values = self.controller.get_hat(self.HAT_DPAD)
            else: self.hat_values = (0, 0)

            # --- Detect Events based on State Changes ---
            event = None

            # LT Press/Release Event (using state)
            ...
            if self.hat_values != self.last_hat_values:
                 # Only trigger HAT_CHANGED if it wasn't None before (avoid triggering on initial read)
                 # And only if the current value isn't center (0,0) unless the previous wasn't either
                 if self.last_hat_values != (0,0) or self.hat_values != (0,0):
                     event = "HAT_CHANGED" # Prioritize LT event if both happen

            return True, event # Success, optional event detected
    ...
    def is_connected(self):
        ...
        return self.connected and self.controller and self.controller.get_init()
...
class LidarVisualizerFrame(ttk.Frame):
    ...
    def update_display(self):
        ...
        # Handle Auto-Stop Assist Logic (moved from LidarVisualizer directly)
        self.handle_auto_stop_assist(closest_distance)

        # --- Drawing on Canvas ---
        self.canvas.delete("scan", "obstacle", "closest") # Clear dynamic elements

        # Draw reference elements (in case canvas resized?) - potentially optimize this
        # self.draw_reference_elements() # Redrawing reference lines can be slow
        ...
        if closest_distance < float('inf'):
            ...
            if closest_distance < threshold:
                is_braking = self.main_app.brake_active # Check central brake status
                if self.stop_assist_var.get() and is_braking:
                     # Check if braking was likely due to auto-assist
                     # This is heuristic, might be manually braked too
                     self.canvas.itemconfig(self.warning_text, text="AUTO-BRAKING ACTIVE!", fill=RED)
                else:
                     self.canvas.itemconfig(self.warning_text, text="BRAKE ADVISED!", fill=YELLOW)
            else:
                self.canvas.itemconfig(self.warning_text, text="")
        else:
            self.canvas.itemconfig(self.distance_text, text="Dist: -- cm")
        ...
        # Use a variable update rate? Faster when close? For now, fixed.
    ...
    def handle_auto_stop_assist(self, closest_distance):
        """Applies brake (S 1) if obstacle is too close and assist is active."""
        ...
        if 0 < closest_distance < threshold:
             # Obstacle is too close, check if brake needs to be applied
             if not self.main_app.brake_active: # Check central brake state
                 self.log_callback(f"Auto-Stop: Obstacle detected at {closest_distance:.1f}cm (Threshold: {threshold}cm). Applying brake (S 1).", is_warning=True)
                 # Send brake command and update central state
                 if self.arduino_communicator.send_command("S 1"):
                     # Update central state ONLY if command sent successfully (avoids desync)
                     self.main_app.update_brake_state(True, "Auto-Stop") # Indicate reason
                 # No need to send V 0 here, S 1 should handle it on ESP32 side
        # --- Auto-Release Logic (Optional - More Complex) ---
        # Should the system automatically release the brake (Q 1) when the path is clear?
        # This can be risky if the path clears momentarily.
        # Let's NOT auto-release for now. User must manually release or use controller.
        # else:
        #     # Path is clear (or no obstacle detected)
        #     if self.main_app.brake_active and self.main_app.brake_reason == "Auto-Stop":
        #          # If brake is active AND was applied by Auto-Stop, release it
        #          self.log_callback(f"Auto-Stop: Path clear. Releasing brake (Q 1).")
        #          if self.arduino_communicator.send_command("Q 1"):
        #              self.main_app.update_brake_state(False, "Auto-Release")
