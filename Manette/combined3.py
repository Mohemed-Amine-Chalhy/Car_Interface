...
class XboxController:
    def __init__(self, controller_id=0, log_callback=None):
        ...
        # --- Controller Mapping (Potential Differences PS5 vs Xbox) ---
        # Xbox Defaults (Keep for reference/fallback)
        self.AXIS_RIGHT_STICK_X_XBOX = 0 # Usually axis 0 on XInput
        self.AXIS_RT_XBOX = 5  # Right trigger (often axis 5, range -1 to 1)
        self.AXIS_LT_XBOX = 4  # Left trigger (often axis 4, range -1 to 1)
        self.HAT_DPAD_XBOX = 0  # D-pad is typically hat 0 on Xbox

        # PS5 Defaults (Common Mappings - ***VERIFY THESE WITH DEBUG PRINTS***)
        # Sticks might be different axes (e.g., 0/1 for Left, 2/3 or 3/4 for Right)
        self.AXIS_RIGHT_STICK_X_PS5 = 0 # Often axis 2 for horizontal on right stick
        self.AXIS_RT_PS5 = 5 # Often axis 5, range -1 to 1 (like L2/R2)
        self.AXIS_LT_PS5 = 4 # Often axis 4, range -1 to 1 (like L2/R2)

        # --- D-Pad Button Mapping (*** MOST IMPORTANT PART TO VERIFY ***) ---
        # Common PS5 button numbers for D-Pad (MUST BE CHECKED)
        ...
        self.rt_value = -1.0 # Triggers often rest at -1
        self.lt_value = -1.0
        self.hat_values = (0, 0) # Store raw hat value if present

        # State tracking for D-Pad buttons and events
        self.dpad_button_state = {'up': False, 'down': False, 'left': False, 'right': False}
        self.last_dpad_tuple = (0, 0) # Store the derived (x, y) tuple for change detection
        self.lt_pressed_state = False # Tracks if LT is currently considered pressed (> threshold)

        self.DEADZONE_STICK = 0.15
        self.DEADZONE_TRIGGER = 0.1 # Triggers might need a smaller deadzone near -1
        ...
        # Cache last sent values to avoid spamming identical commands
    ...
    def _determine_mappings(self):
        ...
        # Refine this logic as needed based on observed behavior
        ...
        # Verify axis numbers are valid for the detected controller
        ...
        if self.AXIS_RT >= num_axes:
            ...
            self.AXIS_RT = max(0, num_axes - 1) # Guess last axis
        if self.AXIS_LT >= num_axes:
            ...
            self.AXIS_LT = max(0, num_axes - 2) # Guess second to last axis
    ...
    def connect(self):
        ...
        try:
            ...
            # Determine mappings based on the connected controller
            self._determine_mappings()

             # Reset states on connect
            ...
            self.dpad_button_state = {'up': False, 'down': False, 'left': False, 'right': False} # Reset button state
            self.last_dpad_tuple = self.get_hat_values() # Calculate initial dpad tuple
        ...
        if self.controller:
            try:
                self.controller.quit()
                 ...
                 self.log_callback(f"Error during controller quit: {e}", is_error=True)
        self.connected = False
        self.controller = None
        self.controller_name = "None"
    ...
    def update(self):
        """Pumps Pygame events, reads controller state, handles button/hat D-pad."""
        if not self.connected or not self.controller:
            # Attempt reconnection?
            # if not self.connect(): # Maybe add a timer to avoid spamming connection attempts
                 return False, None # Return False if not connected

        event_occurred = None # Track if a relevant event happened this update cycle
        dpad_input_changed = False # Specifically track if D-Pad state changed

        try:
            # --- Process Pygame Event Queue ---
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    # Signal application to close? Or handle elsewhere?
                    self.log_callback("Pygame quit event received.")
                    # Potentially call self.disconnect() or a main app shutdown function
                    return False, None
                    ...
                    # May need re-initialization logic here if connection was lost
                    if not self.controller.get_init():
                        self.connect() # Try to reconnect fully
                    ...
                    self.disconnect()
                    return False, None # Critical event, stop update cycle

                # --- Button Events (Handles D-Pad for PS5-like controllers) ---
                elif event.type == pygame.JOYBUTTONDOWN:
                    button = event.button
                    # --- Add this Debug Print to find your D-Pad button numbers ---
                    # self.log_callback(f"DEBUG: Button {button} Pressed")
                    # ---
                    pressed = True
                    if not self.USE_HAT_DPAD: # Only process as D-Pad if using button mode
                    ...
                    # Add other button down handlers here if needed (e.g., for LT if it's a button)


                elif event.type == pygame.JOYBUTTONUP:
                    button = event.button
                    # --- Add this Debug Print ---
                    # self.log_callback(f"DEBUG: Button {button} Released")
                    # ---
                    pressed = False
                    if not self.USE_HAT_DPAD: # Only process as D-Pad if using button mode
                        if button == self.BUTTON_DPAD_UP: self.dpad_button_state['up'] = pressed; dpad_input_changed = True
                    ...
                    # Add other button up handlers here


                # --- Hat Events (Original D-Pad for Xbox) ---
                elif event.type == pygame.JOYHATMOTION:
                     if self.USE_HAT_DPAD and event.hat == self.HAT_DPAD_XBOX:
                          # If using hat mode AND the hat moved, update raw value
                          self.hat_values = event.value
                          dpad_input_changed = True # Hat movement also changes D-Pad state


                # --- Axis Events ---
                elif event.type == pygame.JOYAXISMOTION:
                    # We read the current state below, but processing events keeps queue clear
                    # Could check specific axes here if needed (e.g., LT/RT if they fire axis events)
                    pass


            # --- Read Current States (Axes) AFTER processing events ---
            # This ensures we have the latest values even if no specific event fired
            # (e.g., holding a stick doesn't continuously fire events)
            num_axes = self.controller.get_numaxes()
            if num_axes > self.AXIS_RIGHT_STICK_X:
                self.right_stick_x = self.controller.get_axis(self.AXIS_RIGHT_STICK_X)
            else: self.right_stick_x = 0.0

            if num_axes > self.AXIS_RT:
                self.rt_value = self.controller.get_axis(self.AXIS_RT)
            else: self.rt_value = -1.0

            if num_axes > self.AXIS_LT:
                self.lt_value = self.controller.get_axis(self.AXIS_LT)
            else: self.lt_value = -1.0

            # --- Determine Combined D-Pad State ---
            current_dpad_tuple = self.get_hat_values() # Calculates based on mode (hat/button)

            # --- Detect D-Pad Change Event ---
            if dpad_input_changed and current_dpad_tuple != self.last_dpad_tuple:
                 # Only trigger event if value actually changed and wasn't/isn't center (unless previous wasn't center)
                 if self.last_dpad_tuple != (0,0) or current_dpad_tuple != (0,0):
                     event_occurred = "HAT_CHANGED" # Reuse existing event name for compatibility
                 self.last_dpad_tuple = current_dpad_tuple # Update last state

            # --- Detect LT Press/Release State Change ---
            # This logic remains the same, based on the continuously read lt_value
            lt_currently_pressed = self.lt_value > 0.5 # Threshold for pressed state
            if lt_currently_pressed and not self.lt_pressed_state:
                # Only trigger LT_PRESSED if D-pad didn't change in the *same* cycle?
                # Or prioritize LT? Let's prioritize LT if both happen.
    ...
    def get_hat_values(self):
        ...
        Calculates based on tracked button state if USE_HAT_DPAD is False.
        """
        if self.USE_HAT_DPAD:
            # Read the current hat state directly if using hat mode
            if self.connected and self.controller and self.controller.get_numhats() > self.HAT_DPAD_XBOX:
                ...
                return (0, 0) # Default if no hat or not connected
        else:
            # Calculate tuple from button states
            dpad_x = 0
            dpad_y = 0
    ...
    # --- Other methods (get_steering_value, get_acceleration_value, etc.) remain the same ---
    # They rely on self.right_stick_x, self.rt_value which are updated correctly in update()
    ...
    def get_steering_value(self):
        ...
        if raw_value > 0: # Right
            ...
            scaled_value = (raw_value + self.DEADZONE_STICK) / (1 - self.DEADZONE_STICK) # Value is negative
            steering = self.STEERING_CENTER + scaled_value * (self.STEERING_CENTER - self.STEERING_MIN) # scaled is negative

        # Clamp and return integer value
    ...
    def get_acceleration_value(self):
        ...
        # Normalize RT from (-1 to 1) range to (0 to 1) range
        normalized = (raw_value + 1) / 2

        # Apply deadzone (near the released state 0)
        if normalized < self.DEADZONE_TRIGGER:
            return 0
    ...
    def has_steering_changed(self):
        """Checks if the calculated steering value differs significantly from the last sent one."""
        current_steering = self.get_steering_value()
        # Use a threshold to avoid sending updates for tiny fluctuations
        if abs(current_steering - self.last_sent_steering) > 5: # Adjust threshold as needed
            self.last_sent_steering = current_steering
            return True
        return False

    def has_accel_changed(self):
        """Checks if the calculated acceleration value differs from the last sent one."""
        current_accel = self.get_acceleration_value()
        if current_accel != self.last_sent_accel:
            self.last_sent_accel = current_accel
            return True
        return False

    def is_connected(self):
        # Double check controller is initialized too
        # Add a check to see if the underlying joystick object still exists
        try:
        ...
        except pygame.error: # Handle cases where joystick[id] is invalid after disconnect
             return False
...
class LidarVisualizerFrame(ttk.Frame):
    ...
    def update_display(self):
        ...
        self.handle_auto_stop_assist(closest_distance) # <<< This now calls emergency_stop()
        ...
        if closest_distance < float('inf'):
            ...
            # ***** CHANGE HERE: Update warning text logic *****
    ...
    def handle_auto_stop_assist(self, closest_distance):
        ...
        if 0 < closest_distance < threshold:
             ...
             # else:
                 # Optional: Log that condition persists but E-Stop already active
                 # print(f"Debug: Auto-Stop condition persists {closest_distance:.1f}cm, E-Stop already active.")
        # --- Auto-Release Logic ( REMOVED - E-Stop requires manual release ) ---
        # No auto-release for E-Stop condition.
...
class CombinedRobotApp:
    ...
    def release_emergency_stop(self):
         """Sends Q 1 to release brake after an E-Stop (optional)."""
         if self.brake_active and self.brake_reason == "E-Stop":
              self.log_message("Releasing E-Stop brake (Q 1)...")
              if self.arduino_communicator.send_command("Q 1"):
                   self.update_brake_state(False, "E-Stop Release")
              else:
                   self.log_message("Failed to send Q 1 command for E-Stop release.", is_error=True)
         else:
              self.log_message("Cannot release E-stop: Brake not active or not due to E-Stop.", is_warning=True)
        ...
        try:
            if self.xbox_controller.is_connected():
                ...
                if not success:
                    ...
                    # Don't schedule next poll if disconnected? Or keep trying? Keep trying.
                    ...
                    # Steering (only send if changed significantly)
                    ...
                    if event == "HAT_CHANGED":
                        ...
                        # Add X axis handling? (hat_x == 1 or -1)

                    elif event == "LT_PRESSED":
                        # If LT is pressed, engage brake
                        if not self.brake_active: # Only engage if not already active
