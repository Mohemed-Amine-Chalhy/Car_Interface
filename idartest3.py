...
class LidarVisualizer:
    ...
    def connect_arduino(self):
        ...
        if port:
            ...
            if self.serial_communicator.connect():
                ...
                self.update_status(f"Error connecting to Arduino on {port}") # Added status update
        else:
    ...
    def on_stop_assist_changed(self):
        if self.stop_assist_var.get():
            # Send 'V 0\n' and 'F\n' commands when checkbox is checked
            if self.serial_communicator.connected:
                ...
                if not self.serial_communicator.f_sent:
                    ...
                    self.update_status("Stop assist activated: V 0 and F commands sent")
        else:
            # Nothing to do when unchecked
